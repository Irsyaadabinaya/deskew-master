...
def determine_skew_dev(
    ...
    img = image
    edges = canny(img, sigma=sigma)                 #Edge filter an image using Canny Algorithm
    out, angles, distances = hough_line(edges)      #perform a straight line Hough Transform
...
def determine_skew(image: np.ndarray, sigma: float = 3.0, num_peaks: int = 20) -> Optional[float]:
    ...
    """
    angle, _, _, (h,a,d) = determine_skew_dev(image, sigma=sigma, num_peaks=num_peaks)
    # display_hough(h,a,d)
    return angle
